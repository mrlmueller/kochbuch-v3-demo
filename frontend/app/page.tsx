...
function DesktopHome({ categories, allRecipes }: { categories: Category[]; allRecipes: RecipeListItem[] }) {
  ...
  const quick = allRecipes.filter(r => r.time_minutes > 0 && r.time_minutes <= 25).slice(0, 4)
  const hearty = allRecipes.filter(r => r.category_slug === 'hauptgerichte').slice(0, 4)
  const sweet = allRecipes.filter(r => r.category_slug === 'backen-und-suesses' || r.category_slug === 'snacks').slice(0, 3)
        ...
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '60px 40px 80px', display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 64, alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 28, marginBottom: 36, color: 'var(--muted)', fontSize: 13.5 }}>
              {featured?.time_minutes > 0 && <span>{featured.time_minutes} Minuten</span>}
